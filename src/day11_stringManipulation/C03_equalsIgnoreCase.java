package day11_stringManipulation;

import java.util.Scanner;

public class C03_equalsIgnoreCase {
    public static void main(String[] args) {

        // Kullaniciya derse katilip katilmak istemedigini sorun
        // evet derse, cevabini ve "derse katiliminiz onaylanmistir"
        // hayir derse cevabini ve "sonraki derslerimize bekleriz" yazdirin

        Scanner scan = new Scanner(System.in);
        System.out.println("Derse katilip katilmak istemediginizi belirtin \nEvet veya Hayir yazin");
        String cevap = scan.next();

        if (cevap.equalsIgnoreCase("evet")) {
            System.out.println("cevabini : "+cevap + " derse katiliminiz onaylanmistir");
        }
        else if (cevap.equalsIgnoreCase ("hayir")) {
            System.out.println("cevabini : "+cevap + " sonraki derslerimize bekleriz");
        }
        else {
            System.out.println("Lutfen evet veya hayir yaziniz");
        }
    }
}
