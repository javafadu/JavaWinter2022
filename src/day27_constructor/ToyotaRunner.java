package day27_constructor;

public class ToyotaRunner {
    public static void main(String[] args) {

    Toyota t1 = new Toyota();
        System.out.println(t1.marka);  // Toyota
        System.out.println(t1.tekerAdedi); // 4
        System.out.println(t1.model);
        t1.model="Corolla";
        t1.yakit="Benzin";
        t1.yil=2022;

        System.out.println("T1 model: "+t1.model +
                ", yakit : "+t1.yakit + ", yil : "+ t1.yil);

        t1.maxHiz();  // Benzinli arabalar max 240 km/h yapar


// T1 model: Corolla, yakit : Benzin, yil : 2022

        Toyota t2 = new Toyota();
        System.out.println(t2.model);  // null
        System.out.println(t2.marka); // Toyota
        t2.model ="Yaris";
        t2.yakit = "Benzin";
        t2.yil = 2021;
        System.out.println();

        System.out.println("T2 model: "+t2.model +
                ", yakit : "+t2.yakit + ", yil : "+ t2.yil);
// T2 model: Yaris, yakit : Benzin, yil : 2021

        t2.renkTercihleri();  // Yaris renk secenekleri : sari ve lacivert

    }
}
